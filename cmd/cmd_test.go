...
func TestFormatPatternForDisplay(t *testing.T) {
	tests := []struct {
		...
		// Patterns without special characters should remain unchanged
		{"error", "error"},
		{"test", "test"},
		{"hello world", "hello world"},
		{"config-map", "config-map"},

		// Patterns with special shell characters should be wrapped in quotes
		{"[Error]", `"[Error]"`},
		{"test*", `"test*"`},
		{"file?", `"file?"`},
		{"hello|world", `"hello|world"`},
...
func TestFormatPatternForDisplay_RandomPatterns(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		// Generate a simple word (should not be quoted)
		simplePattern := gofakeit.Word()
		result := formatPatternForDisplay(simplePattern)
		if result != simplePattern {
			t.Errorf("Simple pattern %q should not be wrapped, got %q", simplePattern, result)
		}
	}
	...
	for _, pattern := range specialPatterns {
		...
		// Patterns with special chars should be wrapped in quotes
		if !strings.HasPrefix(result, `"`) || !strings.HasSuffix(result, `"`) {
			t.Errorf("Pattern %q with special chars should be wrapped in quotes, got %q", pattern, result)
		}
	}
}
