...
// WrapKubernetesError takes an error returned from a Kubernetes API call and returns a more user-friendly error message if it recognizes the error type. If the error is not recognized, it returns the original error.
func WrapKubernetesError(err error) error {
	...
	if strings.Contains(errStr, "no configuration has been provided") ||
		...
		strings.Contains(errStr, "invalid configuration") ||
